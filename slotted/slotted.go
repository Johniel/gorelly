...
type Header struct {
	NumSlots        uint16 // Number of slots (records) in the page
	FreeSpaceOffset uint16 // Offset to the start of free space
	Pad             uint32 // Padding for alignment
}

// Pointer points to a record in the data area.
// It stores the offset and length of the record.
type Pointer struct {
	Offset uint16 // Offset from the start of the body
	Len    uint16 // Length of the record
}
...
// It manages variable-length records stored in a page with a header and pointer array.
...
//   - body[FreeSpaceOffset:end]: Data records (variable-length records stored backwards)
//
// Example layout (body size = 100, 2 records: "hello" and "world"):
//
...
type Slotted struct {
	header   *Header
	body     []byte    // Body contains: [pointers array][free space][data records]
...
func NewSlotted(bytes []byte) *Slotted {
	...
	if numPointers > 0 {
		...
		for i := 0; i < numPointers; i++ {
			offset := binary.LittleEndian.Uint16(pointersData[i*pointersSize:])
			len := binary.LittleEndian.Uint16(pointersData[i*pointersSize+2:])
...
func (s *Slotted) Insert(index int, dataLen int) bool {
	...
	if cap(s.pointers) < s.NumSlots() {
		newPointers := make([]Pointer, s.NumSlots())
